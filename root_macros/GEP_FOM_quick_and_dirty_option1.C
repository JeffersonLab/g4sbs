...
void GEP_FOM_quick_and_dirty(const char *configfilename, const char *outfilename="GEP_FOM.root"){
  ...
  gep_tree_option1 *T = new gep_tree_option1(C);
  ...
  while( T->GetEntry( elist_temp->GetEntry( nevent++ ) ) ){
    ...
    // for( int itrack=0; itrack<T->Harm_FPP11_Track_ntracks; itrack++ ){
    //   int MID = (*(T->Harm_FPP11_Track_MID))[itrack];
    //   if( MID == 0 || T->Harm_FPP11_Track_ntracks == 1 ) {
    ...
    // for( int itrack=0; itrack<T->Harm_FPP12_Track_ntracks; itrack++ ){
    //   int MID = (*(T->Harm_FPP12_Track_MID))[itrack];
    //   if( MID == 0 || T->Harm_FPP11_Track_ntracks == 1 ) {
    // 	idx_FPP2_track = itrack;
    // 	break;
    //   }
    // }
    //if( T->Harm_FPP12_Track_ntracks == 1 ) idx_FPP2_track = 0;
	...
	//   TVector3 FPP2_track( (*(T->Harm_FPP12_Track_Xp))[idx_FPP2_track],
	// 		       (*(T->Harm_FPP12_Track_Yp))[idx_FPP2_track],
	// 		       1.0 );
