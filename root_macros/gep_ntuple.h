...
// Mon Apr  3 12:08:44 2023 by ROOT version 6.26/10
// from TTree T/Geant4 SBS Simulation
// found on file: GEP12_job_1.root
//////////////////////////////////////////////////////////
...
#ifdef gep_ntuple_cxx
gep_ntuple::gep_ntuple(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("GEP12_job_1.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("GEP12_job_1.root");
      }
      f->GetObject("T",tree);
...
gep_ntuple::~gep_ntuple()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t gep_ntuple::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t gep_ntuple::LoadTree(Long64_t entry)
{
...
void gep_ntuple::Init(TTree *tree)
{
...
void gep_ntuple::Show(Long64_t entry)
{
...
Int_t gep_ntuple::Cut(Long64_t entry)
{
