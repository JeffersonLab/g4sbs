...
//void gep_updating_trigger_analysis_L2_generic( const char *rootfilename, const char *outputfilename="new4_gep3_trigrates_temp.root", double ECAL_threshold=0.8, double HCAL_threshold=0.5, int xsecflag=2, const char *logicfilename_ecal="GEP_ECAL_L1sums.txt", const char *thresholdfilename_ecal="gep3/final_ecal_trigger_temp.txt", const char *thresholdfilename_hcal="gep3/final_hcal_trigger_temp.txt", int pheflag=0, const char *assocfilename="gep3/HcalEcalBinLookup.txt" ){

void gep_updating_trigger_analysis_L2_generic( const char *rootfilename, const char *outputfilename, double ECAL_threshold=0.8, double HCAL_threshold=0.5, int xsecflag=2, const char *logicfilename_ecal="GEP_ECAL_L1sums.txt", const char *thresholdfilename_ecal="gep3/final_ecal_trigger_temp.txt", const char *thresholdfilename_hcal="gep3/final_hcal_trigger_temp.txt", int pheflag=0, const char *assocfilename="HcalEcalBinLookupNewJan18.txt" ){
  ...
  int nrows_hcal=24;
  int ncols_hcal=12;
  for( int row=1; row<=nrows_hcal-2; row++ ){
    for( int col=1; col<=ncols_hcal-2; col++ ){
      list_of_nodes_hcal.insert( current_node );
      for( int m=col; m<=col+2; m++ ){ //Add all blocks in each 3x3 sum to the current node:
	for( int n=row; n<=row+2; n++ ){ //Add
  ...
  */
  //need to refer to bins instead of nodes due lookup table using bin groupings, wiht nodes inside bins
  bool use_ECAL_HCAL_associations=false;
  map<int, set<int> > ECAL_bins_HCAL;
  if( assocfile ){
    while( !assocfile.eof() ){
      int hcalbin, N;
      assocfile >> hcalbin >> N;
	...
	ECAL_bins_HCAL[hcalbin].insert(ecalbin);
      }
    }
  }
  ...
  ////////////////////////////////////////////////////////Node to Bin Groupings////////////////////////////////////////////////////////////////////////




  std::vector< std::vector<int> > hcalBins(129);//bin 0 is empty, NOT TRUE, BIN 0 IS FIRST BIN, BIN 127 IS THE LAST ONE, BIN 128 IS EMPTY
  std::vector< std::vector<int> > ecalBins(896);



  //get which nodes are grouped together into which bins

  ifstream hcalfile("hcalBinGroups.txt");

  TString hcalcurrentline;

  while( hcalcurrentline.ReadLine(hcalfile) ) {
    cout << hcalcurrentline << endl;
  
    if( !hcalcurrentline.BeginsWith( "#" ) ){ //new cell information:
      TObjArray *tokens = ( (TObjArray*) hcalcurrentline.Tokenize(",") );
      int ntokens = tokens->GetEntries();

      int bin = ( (TObjString*) (*tokens)[0] )->GetString().Atoi();

      for(int i = 1; i < ntokens; i++){

	int node = ( (TObjString*) (*tokens)[i] )->GetString().Atoi();

	hcalBins[bin].push_back(node);
  ...
  //get which ecal nodes are grouped together in which bins

  ifstream ecalfile("ecalBinGroupsNew.txt");

  TString ecalcurrentline;

  while( ecalcurrentline.ReadLine(ecalfile) ) {
    cout << ecalcurrentline << endl;
  
    if( !ecalcurrentline.BeginsWith( "#" ) ){ //new cell information:
      TObjArray *tokens = ( (TObjArray*) ecalcurrentline.Tokenize(",") );
      int ntokens = tokens->GetEntries();
  ...
  for( nevent=0; nevent<C->GetEntries(); ++nevent ){
	...
	//////////////finding which bin corresponds to inode in order to use bin lookup table//////////////////////////////////////////////////////////////////////////



	//Find which bin this hcal node is in, in order to use bin lookup table 
	int tempHBin;
	bool binFound = false;

	for(int iBin = 0; iBin < hcalBins.size(); iBin++){

	  for(int jBin = 0; jBin < hcalBins[iBin].size(); jBin++){

	    if(hcalBins[iBin][jBin] == *inode){
	      tempHBin = iBin;
	      binFound = true;
	    }
	    if(binFound == true){
	...
	/*
	  ...
	  for( set<int>::iterator enode = list_of_nodes_ecal.begin(); enode != list_of_nodes_ecal.end(); ++enode ){
	    //Find which bin this ecal node is in, in order to use bin lookup table
	    int tempEBin;
	    bool ebinFound = false;

	    for(int iBin = 0; iBin < ecalBins.size(); iBin++){

	      for(int jBin = 0; jBin < ecalBins[iBin].size(); jBin++){

		if(ecalBins[iBin][jBin] == *enode){
		  tempEBin = iBin;
		  ebinFound = true;
		}
		if(ebinFound == true){
	...
	for( set<int>::iterator ebin = ECAL_bins_HCAL[tempHBin].begin(); ebin != ECAL_bins_HCAL[tempHBin].end(); ++ebin ){
	  //now for( all nodes in this bin )
	  //for( set<int>::iterator elnode = ecalBins[*ebin].begin(); elnode != ecalBins[*ebin].end(); ++elnode ){
	  for(int node = 0; node < ecalBins[*ebin].size(); node++){

	    int elnode = ecalBins[*ebin][node];
  
	    if( node_sums[ elnode ]/logic_mean_ecal[elnode] > ECAL_threshold ){ //this ECAL sum fired:
	...
	for( int ebin=1; ebin<=hrate_vs_threshold_ECAL->GetNbinsX(); ebin++ ){
	  ...
	  for( int enode=0; enode<trigger_nodes_fired[ebin-1]; enode++ ){
		...
		if(ecalBins[iBin][jBin] == ecalnode){
		  tempEBin = iBin;
		  ebinFound = true;
		}
		if(ebinFound == true){
	    ...
	    //code above:need to go from ecal node to ecal bin to use lookup table

	    //  if( ECAL_nodes_HCAL[*inode].find( ecalnode ) != ECAL_nodes_HCAL[*inode].end() ){
	    if( ECAL_bins_HCAL[tempHBin].find(tempEBin) != ECAL_bins_HCAL[tempHBin].end() ){ 
	      nECALtemp++;
	    }
	  }
	  if( nECALtemp > 0 ){
	...
	for( int jthr=1; jthr<=hrate_vs_threshold_ECAL->GetNbinsX(); jthr++ ){
	  ...
	  for( int hnode=0; hnode<trigger_nodes_fired_hcal[ithr-1]; hnode++ ){
	    ...
	    //Find which bin this hcal node is in, in order to use bin lookup table 
	    int newHBin;
	    bool newbinFound = false;

	    for(int iBin = 0; iBin < hcalBins.size(); iBin++){

	      for(int jBin = 0; jBin < hcalBins[iBin].size(); jBin++){

		if(hcalBins[iBin][jBin] == hcalnode){
		  newHBin = iBin;
		  newbinFound = true;
		}
		if(newbinFound == true){
		...
		if(newebinFound == true){
	      ...
	      if( ECAL_bins_HCAL[newHBin].find(newEBin) != ECAL_bins_HCAL[newHBin].end() ){ // Real coincidence!
		anycoin_ij = true;
	      }
	    }
