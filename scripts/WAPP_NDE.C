...
void WAPP_FOM_quick_and_dirty(const char *configfilename, const char *outfilename="WAPP_FOM.root"){
  ...
  TH1D *hEgamma = new TH1D("hEgamma","E_{#gamma} (GeV)",500,0.5,10.5);
  TH1D *hEgamma_pion = new TH1D("hEgamma_pion", ";E_{#gamma}(p_{#pi},#theta_{pi});",500,0.5,10.5);
  ...
  TH1D *hMX2_pi = new TH1D("hMX2_pi",";Missing mass squared (#gamma #pi^{+});", 250,0,2);
  ...
  while( T->GetEntry( elist_temp->GetEntry( nevent++ ) ) ){
    ...
    TLorentzVector Pbeam( 0, 0, Ebeam_current, Ebeam_current );

    TLorentzVector Pneutron_pigamma_smear = PN + Pbeam - Ppi_lab_smear;
    
    double MX2_gammapi = Pneutron_pigamma_smear.M2();
    ...
    if( Ebeam_current-Egamma_ppi_smear < 0.12 ) hMX2_pi->Fill( MX2_gammapi, weight );
    ...
    hEgamma_pion->Fill( Egamma_ppi_smear, weight );
