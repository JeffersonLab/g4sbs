...
// Thu Apr 21 20:21:45 2022 by ROOT version 6.24/06
// from TTree T/Geant4 SBS Simulation
// found on file: gep_BB_10GeV2_job1.root
//////////////////////////////////////////////////////////

#ifndef gep_BB_tree_h
#define gep_BB_tree_h
...
#ifdef gep_BB_tree_cxx
gep_BB_tree::gep_BB_tree(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("gep_BB_10GeV2_job1.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("gep_BB_10GeV2_job1.root");
      }
      f->GetObject("T",tree);
...
gep_BB_tree::~gep_BB_tree()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t gep_BB_tree::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t gep_BB_tree::LoadTree(Long64_t entry)
{
...
void gep_BB_tree::Init(TTree *tree)
{
...
void gep_BB_tree::Show(Long64_t entry)
{
...
Int_t gep_BB_tree::Cut(Long64_t entry)
{
