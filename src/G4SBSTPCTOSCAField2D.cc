...
// R.Montgomery included into G4SBS 22/03/18
// R.Montgomery filename for field map hardcoded right now
// field map must be in same directory as g4sbs executable

#include "G4SBSTPCTOSCAField2D.hh"

// G4SBSTPCTOSCAField2D::G4SBSTPCTOSCAField2D( const char* filename, double xoff, double yoff,
// 			    double zoff, double scale) 
G4SBSTPCTOSCAField2D::G4SBSTPCTOSCAField2D(double xoff, double yoff,
			    double zoff, double scale) 
{    
  double lenUnit   = cm;
  ...
  printf("Reading solenoid TOSCA generated field map from file \n");
  // Open the file for reading.
  // FILE* file = fopen(filename,"r");
  FILE* file = fopen("sol_map_03.dat","r");
  if(!file){
    // printf(" Error opening solenoid tosca file %s\n Abort Geant-4 session\n",filename);
    printf("Error opening solenoid tosca file \n Abort Geant-4 session\n");
    exit(-1);
  }
  char* fl;
  ...
  for (ix=0; ix<nx; ix++) {
	...
	if(!(fl = fgets(buffer,256,file)) ){
	  ...
	  printf("Premature end of solenoid tosca file\n");
	  fclose(file);
	  exit(-1);
	}
	if( sscanf(buffer,"%lf%lf%lf%lf%lf%lf%lf",
	  ...
	  printf("File format for solenoid error, exiting\n");
	  fclose(file);
	  exit(-1);
	}
	bx = bx * scale;
